  ...
  try {
    ...
    if (rows.length === 0) {
      try {
        ...
        console.error('插入默认数据失败:', e.message);
      }
      return getDefaultDramas();
    }
...
export async function getFeaturedDrama() {
  const sql = getSql();
  if (!sql) {
    const defaults = getDefaultDramas();
    return defaults.find(d => d.featured) || defaults[0] || null;
  }
...
export async function getDramaById(id) {
  ...
  if (!sql) {
    const defaults = getDefaultDramas();
    return defaults.find(d => d.id === numId) || null;
  }
...
export async function updateDrama(id, updates) {
  ...
  try {
    ...
    // 使用 ?? 代替 ||，避免 featured=false / episodes=0 / 空字符串被默认值覆盖
    const rows = await sql`
      UPDATE dramas SET
        title = ${updates.title ?? ''},
